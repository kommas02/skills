...
2) Identify the right database/template in `references/` (team wiki, how-to, FAQ, decision log, learning, documentation).
...
- Pick the correct database using `references/*-database.md` guides; confirm required properties (title, tags, owner, status, date, relations).
...
- Use templates in `references/` to structure content (section headers, checklists).
...
- `references/` — database schemas and templates (e.g., `team-wiki-database.md`, `how-to-guide-database.md`, `faq-database.md`, `decision-log-database.md`, `documentation-database.md`, `learning-database.md`, `database-best-practices.md`).
